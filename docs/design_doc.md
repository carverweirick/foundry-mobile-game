...
**Grid and station footprints:** the floor is meant to sit on a tile grid rather than freeform pixel placement, the concrete version of the Floor Editor's grid based vision above. Every station has a footprint, how many grid cells it occupies, standardized and kept small enough that multiple stations can eventually sit side by side in a room rather than each one claiming as much space as it does today. Every station also has a single fixed access point, one specific tile directly in front of its footprint, the exact spot an interaction or a technician's arrival actually happens. For a first pass every station faces the same direction (south, toward the camera), no rotation, which keeps the art simple, one facing per station type, while still giving pathfinding somewhere concrete to route toward instead of a station's center.

**First real step taken this session** (direct request: "turn the game into more of a grid feeling so that customization of floor layout later on will be easy/contained for the user"): a real 20x20 grid unit now underlies the floor - every room boundary and every station position is expressed in whole grid-cell multiples, and the Print Room actually renders its floor as tiles (the previously-unused sliced tileset asset, one uniform tile for now) scaled so each tile occupies exactly one grid cell. What's still open: stations don't yet declare an explicit multi-cell footprint size (today "how much space a station takes" is still implicit in its sprite, not a real grid-cell rectangle), there's no fixed single access point per station yet, no rotation/facing system, and critically, no player-facing editor at all - this is the underlying grid the future Floor Editor would sit on top of, not the editor itself.
...
**Gems, a second "harder to get" currency (built this session, direct request):** unlike currency, gems aren't routine income - the only source right now is a small reward every time Factory Level goes up (see Section 21.2), a genuine milestone rather than something contract volume alone produces. Gems exist specifically to "finish upgrades, fill in for normal currency if you dont have enough for a purchase" - every real purchase in the game (station tier/rack upgrades, technician/specialist hires, printer purchases, Mortar Patch, Redesign) now pays with gold first and tops up any shortfall with gems at a fixed exchange rate, rather than being blocked outright by a gold shortfall the way every purchase used to be. **Built:** the currency itself, the milestone-only earn source, and the gold-plus-gems-covers-the-shortfall spend path across every purchase in the game. **Not yet built:** any other way to earn gems (a live-events/IAP source, matching how a real premium currency usually also has a real-money purchase path, isn't modeled), and any UI showing the gold/gem split a purchase will actually use before you click it - buttons still just show the plain gold price.
...
- ~~Factory level, which gates the printer purchase cap, never actually rises past 1~~ - **done.** Factory Level now rises from a lifetime EXP total earned by completing contracts (see Section 21.2) - the printer cap table is populated through Level 5, though nothing else is gated by Factory Level yet besides the still-unbuilt Air Melter idea (Section 5).
...
5. ~~Decide what actually raises factory level~~ - **done.** Factory Level now rises from a lifetime EXP total earned by completing contracts (bigger/harder tiers award more), with no currency step at all - see Section 21.2's own update.
...
- The number of printers a player is allowed to own is capped by factory level (example given: a Level 1 factory allows 2 printers). This cap is a new progression gate that does not exist anywhere else in the current systems and needs its own data (factory level → printer cap table) - **built**, populated through Level 5 (2/3/4/5/6 printers), a first-pass extension of the one number the doc actually gives.
- Implementation implication: Printing can no longer be a single `Station` node/instance the way it is today. It needs to become a collection of independently-tiered station instances, spawnable up to a cap, likely still using the same reusable Station scene per Section 18's data-driven approach, just multiple instances of it. **Built.**
- **What raises factory level - built, direct design decision (Section 20 item 5 was previously open):** a lifetime EXP total, earned only by completing contracts, never spent, with no currency step at all. Every contract tier awards a different amount on completion (Local Shops/Regional/Industrial/Flagship: 10/25/60/150 EXP, first-pass placeholders), regardless of whether the contract also happened to finish on time - a broader condition than Reputation's own on-time-only gate. Factory Level rises automatically the moment cumulative EXP clears each level's threshold (0/150/400/900/1800 for Levels 1-5), and a single large contract can cross more than one threshold at once. This was a genuine three-way design fork - the alternatives considered were a milestone-then-currency-purchase hybrid (mirroring the R&D Lab's own "unlocks after 200 castings shipped" gate, Section 12) and a pure currency purchase (same pattern as every other upgrade in the game) - both ruled out in favor of a pure EXP mechanic with no spending involved. Each level gained also awards a small amount of the new Gems currency (Section 11) - the one gem source that exists so far, chosen specifically because it's a genuine milestone rather than routine income.
...
**Naming note, added later (this session's staff rework, Section 25): "Engineer" ended up meaning something different from what's described here.** A later session introduced `Technician.StaffRole.ENGINEER` as a genuinely new *role on the Technician class itself* (hired/wagd/assigned/walks around exactly like a Technician, covering Printing/Shelling/Pour skill instead of Patching/Post-Process) - not a tiered version of the existing one-time `SpecialistType` hire this section is actually about. `SpecialistType` (Shell/Pour/Pattern) is untouched and still has no tiers - this idea is still fully open, just needs a different name than "engineer" going forward to avoid confusion with Section 25's own Engineer role.
...
## 25. Session Changes: Staff Roles, Per-Worker Familiarity, and Dark UI Reskin (Implementation Notes)

*Added this session, following a Game-Dev-Tycoon-style "Hire Technicians" mockup and a direct request to mimic its style. Turned out to be a much larger, coherent redesign once clarified across three follow-up questions - see below. Implemented as 3 independently-committed phases (A: theme, B: data model, C: hiring UI), same rhythm as the rest of this session.*

### 25.1 Phase A: whole-game dark industrial theme reskin - built

The single shared `resources/theme/ui_theme.tres` Theme resource (applied project-wide, no scene has its own override) was recolored from the light/parchment palette to dark charcoal panels/buttons with near-black borders and warm off-white text - same shape language (sharp corners, no anti-aliasing, chunky borders, `m5x7.ttf`) throughout, only colors moved. Hover/pressed color *roles* are unchanged - gold still means hover/selected, ember orange still means pressed, keeping the original "heating up" metaphor. The ~16 hardcoded per-node/per-script color overrides a Theme swap alone can't reach (identical rust-brown section headers across 3 `.tscn` files and 4 overlay scripts, plus 3 risk-badge colors and a geometry-icon border color in `contracts_overlay.gd`) were all updated to match. Out of scope: the shop floor's own room-tint palette and the per-geometry-family icon colors (already legible on either background).

### 25.2 Phase B: staff roles, per-worker familiarity, and a new Grinding station - built

Direct request, after a clarifying question about what Patching/Post-Process skill should actually affect (Patching/Post-Process had no defect risk at all before this): *"i want to add grinding to the post process room. grinding can cause a defect. printing, shelling, and pour will be engineer skills. lower level technicians have a higher chance to create defects on parts. would a viable skill rating and familiarity rating be a technician has individual familiarity rankings with different geometries. each time they process a part of a certain geometry they gain some amount of familiarity with that geometry. and the familiarity rating you see on the contract is the average familiarity of your technicians and engineers as well as R&D eventually when we build that out. technicians and engineers you hire, the skill rating you see is the familiarity baseline they have of all geometries."*

- **New Grinding station** (`scenes/station.gd`/`autoload/game_data.gd`), a plain `QUEUE` station in Post Processing between Deshell and Abrasive Blast (matches real investment-casting order). Rolls `DefectCategory.INCLUSION` - the enum's one long-orphaned category (Deshell/Abrasive Blast, its only past sources, lost defect risk entirely in an earlier session) - at a first-pass 0.12 base risk. No Specialist covers Inclusion, a known pre-existing gap.
- **`Technician.StaffRole { TECHNICIAN, ENGINEER }`**, a new role on the *same* class - not a new class, and not a repurposing of the separate one-time `SpecialistType` hire (see 24.7's naming-collision note above). Engineers are hired/wagd/assigned/walk around exactly like Technicians; `role` only changes which departments they roll skill in (`ENGINEER_DEPARTMENTS = [printing, shelling, pour]`, `TECHNICIAN_DEPARTMENTS = [patching, post_process]`) and which stations that skill protects. Burnout and Mold Prep are deliberately absent from both lists - the user named exactly 5 departments, so those two keep their existing tier-only `defect_multiplier` + shop-wide `geometry_familiarity` behavior (Section 21.7) completely unchanged. `SkillTier` (and its hire cost/wage/base defect-multiplier tables) stays shared by both roles - tier is orthogonal to role.
- **Per-department random skill** (`Technician.department_skill`, rolled once at hire via a tier-scaled random range per department - "random in the skills they possess," not deterministically uniform even within one tier) and **per-worker per-geometry familiarity** (`Technician.geometry_familiarity`, starts empty, falls back to `department_skill` as a baseline until real hands-on repetition raises it - "the skill rating you see IS the familiarity baseline"). `Station._gain_worker_experience()` credits the specific `active_worker` once per completed run at a department-mapped station, regardless of outcome (including Patching, which never rolls a defect itself but is still a Technician department).
- **The defect-roll formula branches on department mapping**: a department-mapped station (Printing/Shelling/Pour/Patching/Grinding) now uses the *active worker's own* familiarity (or a shop-wide average if unstaffed) in place of the old shop-wide per-station lookup; Burnout/Mold Prep are untouched, still on the old system.
- **The contract-facing familiarity numbers now average across staff, not stations** - `average_familiarity_stars()`/`weakest_familiarity_stars()` keep their exact old signatures (every existing display call site needed zero changes) but now average/min across every hired Technician+Engineer's own familiarity for that geometry instead of across 4 tracked stations. R&D's own eventual contribution (mentioned in the same request) isn't built yet (Section 12).
- Any staff member can still be assigned to any station (no hard role-based lock) - an out-of-department worker just has 0 baseline skill/familiarity there, same as a total novice, rather than being blocked from the assignment checkbox entirely.

### 25.3 Phase C: rotating applicant pool for hiring - planned, not yet built as of this writing

Replaces "hire any tier, any time" with a `GameData.applicant_pool` mirroring the existing `contract_offers` pattern exactly (roll into a pool, player accepts/hires, a `*_changed` signal, `_click_in_progress()`-guarded overlay refresh): a small pool of randomly-generated named candidates (random role, tier, and per-department skill via Phase B's own roll), a manual gem-cost "Refresh Applicants" reroll, and an automatic refill over time. The Staff overlay's Hire section gets replaced with a persistent-widget-per-applicant list (name, role+tier, per-department stars, hire cost, wage) built directly in the Phase A dark theme.
